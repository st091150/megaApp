...
static const char *UNKNOWN_TASK_PARAMS_ERROR_MSG = "Произошла неизвестная ошибка.";
static const char *INPUT_PATH_EMPTY_ERROR_MSG = "Входная папка не указана.";
static const char *INPUT_FOLDER_DOES_NOT_EXIST_MSG = "Входная папка не существует.";
static const char *OUTPUT_EMPTY_PATH_ERROR_MSG = "Выходная папка не указана.";
static const char *OUTPUT_PATH_CREATE_ERROR_MSG = "Не удалось создать выходную папку.";

inline QString invalidHexKeyErrorMsg() {
    static const QString INVALID_HEX_KEY_ERROR_MSG =
        QString("HEX ключ должен быть ровно %1 байт (%2 hex-символов).").arg(XOR_KEY_BYTES).arg(MAX_HEX_KEY_INPUT_LENGTH);
...
static const char *USER_STOP_REQUEST_MSG = "Завершение выполнение пользователем";
...
inline QString hexKeySizeErrorMsg() {
    static const QString HEX_KEY_SIZE_ERROR =
        QString("Неверный размер ключа (ожидаемый размер %1)").arg(ALGORITHM_KEY_SIZE);
...
static const char *CANT_DELETE_INPUT_FILE_MSG = "Не удалось удалить входной файл";
static const char *TEMP_FILE_DELETE_ERROR_MSG = "Не удалось удалить старый файл";
