...
Logger::Logger(QTextEdit *uiLogTextEditObj, const QString &baseLogPath,
               int maxFileSize, int maxFileCount)
    : _uiTextEditLog(uiLogTextEditObj), _outputMode(Both),
      _maxFileSize(maxFileSize), _maxFileCount(qMax(1, maxFileCount)),
      _currentIndex(0) {
...
void Logger::setOutputMode(EOutputMode mode) { _outputMode = mode; }

Logger::EOutputMode Logger::outputMode() const { return _outputMode; }
...
QString Logger::currentFileName() const {
  QDir dir(_basePath);
  return dir.filePath(
      QString("%1.%2.log").arg(BASE_LOG_FILE_NAME).arg(_currentIndex));
}
...
void Logger::log(const QString &msg, ELogLevel level) {
  if (_logFile.isOpen() && _maxFileSize > 0 &&
      _logFile.size() + msg.size() > _maxFileSize) {
    rotateLogs();
  }

  QString logMsg = QString("[%3] [%1] %2").arg(levelToString(level), msg);
